...
YTTreatmentHelper.SingleVideo = {
  ...
  createHelperPanel: function() {
    ...
    document.getElementById('calculate-btn').addEventListener('click', async () => {
      ...
      const treatmentDate = YTTreatmentHelper.Utils.formatDateToYYYYMMDD(treatmentDateDDMMYYYY);
      if (!treatmentDate) {
      ...
      if (videoPublishDate) {
        console.log(`✅ Video publish date detected: ${YTTreatmentHelper.Utils.formatDate(videoPublishDate)}`);

        // Set minimum date for treatment date picker to publish date
        const treatmentDateInput = document.getElementById('treatment-date');
        const publishDateStr = YTTreatmentHelper.Utils.formatDate(videoPublishDate);
      ...
      if (videoPublishDate) {
        ...
        if (treatment < publish) {
          ...
          console.error(`❌ Invalid: Treatment date ${YTTreatmentHelper.Utils.formatDate(treatment)} is before publish date ${YTTreatmentHelper.Utils.formatDate(publish)}`);
          ...
          document.getElementById('post-start').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(treatmentDate);
      ...
      let ranges;
      try {
        ranges = YTTreatmentHelper.API.calculateDateRanges(treatmentDate, videoPublishDate);
      } catch (error) {
        alert(error.message);
        return;
      }
      ...
      document.getElementById('pre-start').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(ranges.pre.start);
      document.getElementById('pre-start').dataset.original = ranges.pre.start; // Keep YYYY-MM-DD for calculations
      document.getElementById('pre-end').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(ranges.pre.end);
      ...
      document.getElementById('post-start').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(ranges.post.start);
      document.getElementById('post-start').dataset.original = ranges.post.start;
      document.getElementById('post-end').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(ranges.post.end);
      ...
      if (videoPublishDate) {
        document.getElementById('pre-start').dataset.videoPublishDate = YTTreatmentHelper.Utils.formatDate(videoPublishDate);
      }
      ...
      if (ranges.pre.days < maxPossiblePostDays && maxPossiblePreDays < maxPossiblePostDays) {
        const warningMsg = `ℹ️ Using ${ranges.pre.days}-day periods (limited by video publish date on ${YTTreatmentHelper.Utils.formatDateDisplay(ranges.videoPublishDate)}). ${maxPossiblePostDays - ranges.pre.days} days of POST data available but unused for fair comparison.`;
      ...
      safeStorage.set({
        ...
        videoPublishDate: videoPublishDate ? YTTreatmentHelper.Utils.formatDate(videoPublishDate) : null
      });
    });
    ...
    function validateAndUpdateDateRanges() {
      ...
      const preStart = YTTreatmentHelper.Utils.formatDateToYYYYMMDD(preStartDD);
      const preEnd = YTTreatmentHelper.Utils.formatDateToYYYYMMDD(preEndDD);
      const postStart = YTTreatmentHelper.Utils.formatDateToYYYYMMDD(postStartDD);
      const postEnd = YTTreatmentHelper.Utils.formatDateToYYYYMMDD(postEndDD);
    ...
    const runExtraction = async (isRetry = false) => {
      ...
      try {
        ...
        if (window.currentExtractionMode === 'lifetime') {
          ...
          document.getElementById('pre-start').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(preStart);
          document.getElementById('pre-end').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(preEnd);
          const preDays = calculateDaysBetween(preStart, preEnd);
          document.getElementById('pre-days').textContent = preDays;

          // POST: publish → today
          document.getElementById('post-start').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(postStart);
          document.getElementById('post-end').value = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(postEnd);
          const postDays = calculateDaysBetween(postStart, postEnd);
          document.getElementById('post-days').textContent = postDays;
      ...
      try {
        ...
        window.completeAnalysisData = {
          ...
          dates: {
            ...
            extractionDate: YTTreatmentHelper.Utils.formatDate(new Date())
          }
        };
    ...
    safeStorage.get(['lastTreatmentDate']).then((result) => {
      if (result.lastTreatmentDate) {
        ...
        if (dateToDisplay.includes('-')) {
          dateToDisplay = YTTreatmentHelper.Utils.formatDateToDDMMYYYY(dateToDisplay);
        }
  ...
  watchForVideoChanges: function() {
    ...
    const checkVideoChange = () => {
      const newVideoId = utils.getVideoIdFromUrl();

      if (newVideoId && newVideoId !== self.currentVideoId) {
        console.log(`📹 Video changed: ${self.currentVideoId || 'none'} → ${newVideoId}`);
        self.currentVideoId = newVideoId;

        // Reset form when video changes
        self.resetFormForNewVideo();
      }
    };

    // Check immediately
    self.currentVideoId = utils.getVideoIdFromUrl();
